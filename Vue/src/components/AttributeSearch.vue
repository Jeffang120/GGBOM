<template>
  <div class="category-parent-search" style="width: 150px;display: inline-block;">
    <el-autocomplete
      v-model="keyword"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const searchCategory = async (keyword) => {
          try {
            // 实际项目中替换为真实API地址
            const response = await axios.get(`/api/exa/querybyname?keyword=${keyword}`);
